...
def gerar_analise(dados, segmento, modelo=None):
    ...
    secoes.append(
        ...
        "(a FORMA é esta, os números são os do período: 'O faturamento cresceu X%, mas os custos cresceram Y%. "
        "Com isso o lucro caiu Z% e a margem passou de A% para B%.'). Nunca copie números de exemplo — "
        "todo número desta seção sai das linhas calculadas deste cliente.\n"
    ...
    secoes.append(
        ...
        "que os dados atribuem a outro), dependência de um único canal de vendas — SÓ quando esse canal passar de "
        "METADE das vendas; abaixo disso não é dependência, e canal que CRESCEU nunca entra aqui, ele é oportunidade — "
    ...
    secoes.append(
        ...
        "CADA META DERIVA DE UMA DAS AÇÕES ACIMA: a meta é o número que aquela ação move. Se uma ação não move "
        "nenhum número do formulário, ela não vira meta — e tudo bem. "
    ...
    resposta = groq_client.chat.completions.create(
        ...
        messages=[{
            ...
            "content": (
                ...
                f"'criar uma ação de giro para o blazer de alfaiataria nos próximos 30 dias', nunca 'otimize o estoque'. "
                f"Número só entra quando existir nos dados ou nas linhas calculadas. "
                ...
                f"- NÚMERO-META (o alvo que VOCÊ propõe, e SÓ sobre margem, lucro, faturamento ou ticket — "
                f"ex.: 'levar a margem de 8,5% para 10%', partindo dos 8,5% que foram calculados): "
                ...
                f"'reprecificar o vestido midi, que é campeão de venda e está com margem apertada', "
                f"'responder o WhatsApp até as 20h no sábado'.\n"
                ...
                f"💸 TODO PERCENTUAL PRECISA DE PONTO DE PARTIDA CALCULADO.\n"
                f"- PERMITIDO: alvo sobre margem, lucro, faturamento e ticket médio — porque o valor de hoje está "
                f"calculado. Exemplo: 'levar a margem de 8,5% para 10%'. O 8,5% veio da conta; o 10% é o alvo que "
                f"você propõe, e isso é legítimo.\n"
                f"- PROIBIDO: percentual sobre estoque, encalhe, desconto ou corte de custo. O formulário não pergunta "
                f"custo unitário, preço atual, quantidade em estoque nem margem por produto — sem ponto de partida, "
                f"'-40% de desconto', 'reduzir o estoque em 50%', 'vender 20% do encalhe' e 'cortar 10% dos custos' "
                f"são palpite com cara de decisão.\n"
                f"- NO LUGAR: escreva a AÇÃO com ALVO e PRAZO, e diga que o número quem define é o dono — 'criar uma "
                f"ação de giro para o blazer de alfaiataria e a saia longa jeans nos próximos 30 dias, definindo o "
                f"desconto depois de olhar a margem e o custo dessas peças'.\n"
